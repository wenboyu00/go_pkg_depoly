...
func initES() {
	err := elasticsearch.InitClientWithOptions(elasticsearch.DefaultClient, []string{"https://127.0.0.1:9200"},
		"elastic", "elastic", elasticsearch.WithScheme("https"))
	if err != nil {
...
func main() {
	...
	*/
	...
	/*
		goods := make([]Goods, 0)
		res, err := esClient.Query(ctx, IndexName, nil, elastic.NewMatchAllQuery(), 0, 20, elasticsearch.WithEnableDSL(true), elasticsearch.WithOrders(map[string]bool{"favorites": true}))
		if err != nil {
			elasticsearch.EStdLogger.Print(err)
		} else {
			if res != nil {
		...
		elasticsearch.EStdLogger.Printf("%+v", goods)
	*/
}
